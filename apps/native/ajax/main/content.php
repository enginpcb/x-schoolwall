...
if ($action == 'upload_post_image') {
    ...
    if (empty($cl["is_logged"])) {
        ...
        if(empty($_POST['url']) || is_url($_POST['url'])) {
            ...
            try {
                require_once(cl_full_path("core/libs/OpenGraph/OpenGraph.php"));

                $og_data = OpenGraph::fetch($og_url);
                ...
                if (not_empty($og_data)) {
                    ...
                    $type             = (isset($og_data->type)) ? $og_data->type : "article";
                    $og_data_vals     = array(
                        'title'       => cl_croptxt($title, 32, '..'),
                        'description' => cl_croptxt($description, 160, '..'),
                        'image'       => $image,
                        'type'        => $type,
                        'url'         => $og_url
                    );
